...
export function Hero() {
  ...
  return (
    <section className="group relative h-[600px] md:h-[700px] overflow-hidden bg-navy-900">
      {/* Background Slider */}
      ...
      {/* Content */}
      <div className="relative z-10 w-full px-4 h-full flex items-center pointer-events-none">
        <div className="max-w-4xl w-full pointer-events-auto mx-auto md:mx-0">
          {/* Dynamic Text */}
          <div className="min-h-[200px] mt-0 md:mt-24 ml-0 md:ml-24 flex flex-col justify-center text-center md:text-left px-4 md:px-0">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <h1 className="text-blue-900 text-xl md:text-3xl lg:text-4xl font-bold leading-tight drop-shadow-lg">
          ...
          {/* Static Buttons */}
          <div className="absolute bottom-24 md:bottom-20 left-0 md:left-32 w-full md:w-auto flex flex-col sm:flex-row items-center gap-3 md:gap-4 z-10 px-12 md:px-0">
            <AppButton
              label={t("about.viewProducts")}
              to="/products"
              size="lg"
              variant="default"
              className="sm:w-auto text-sm md:text-base py-2 md:py-3 h-10 md:h-12"
            />
            <AppButton
              ...
              size="lg"
              variant="outline"
              className="sm:w-auto text-sm md:text-base px-8 md:py-3 h-10 md:h-12"
            />
          </div>
        </div>
      </div>
      ...
      {/* Pagination - Dots */}
      <div className="absolute bottom-1 left-0 w-full z-20 flex justify-center py-2 md:py-4">
        <div className="flex gap-2 md:gap-4 bg-white/10 backdrop-blur-md px-4 md:px-6 py-2 md:py-2.5 rounded-full border border-white/10 shadow-lg">
          {heroSlides.map((_, index) => (
            <button
              ...
              className={`rounded-full transition-all duration-300 ease-out ${
                index === slideIndex
                  ? "bg-white h-2 w-8 md:h-2 md:w-8"
                  : "bg-white/40 h-2 w-2 md:h-2 md:w-2 hover:bg-white/70"
              }`}
      ...
      <div className="absolute top-1/2 px-10 left-0 flex items-center w-full justify-between gap-4 md:gap-12 mt-6 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <button
          onClick={(e) => {
            e.stopPropagation();
            paginate(-1);
