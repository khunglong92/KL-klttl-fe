...
import type { PriceQuote } from "@/services/api/priceQuotesService";
...
export function PriceQuoteCard({
  ...
  const id = priceQuote?.id;
  const title = priceQuote?.title;
  const subtitle = priceQuote?.subtitle;
  const image = priceQuote?.image;
  return (
    <motion.div
    ...
    >
      ...
      <Link
        to="/quote/$priceQuoteId"
        params={{ priceQuoteId: id || "" }}
