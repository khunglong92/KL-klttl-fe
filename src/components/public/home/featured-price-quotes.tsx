...
import { useFeaturedPriceQuotes } from "@/services/hooks/usePriceQuotes";
import { Link } from "@tanstack/react-router";
import { PriceQuoteCard } from "./price-quote-card";
import { useTheme } from "@/hooks/useTheme";
import { cn } from "@/lib/utils";

export function FeaturedPriceQuotes() {
  const { t } = useTranslation();
  const { theme } = useTheme();
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const { data: allPriceQuotes, isLoading } = useFeaturedPriceQuotes(10);
  ...
  if (!allPriceQuotes || allPriceQuotes.length === 0) {
    return null;
  }

  return (
    <section
      id="featured-price-quotes"
      className={cn(
        "py-16 md:py-24",
      ...
      <div className="w-full px-4">
        ...
        <div className="relative group">
          ...
          <button
            ...
            aria-label="Previous price quotes"
          >
          ...
          <button
            ...
            aria-label="Next price quotes"
          >
            <ChevronRight className="w-6 h-6 stroke-2" />
          </button>

          {/* Price Quotes Horizontal Scroll */}
          <div
          ...
          >
            {allPriceQuotes?.map((priceQuote, index) => (
              <PriceQuoteCard
                key={priceQuote.id}
        ...
        >
          ...
          >
            ...
            >
              {t("priceQuotes.viewAll", "Xem tất cả báo giá")}
