    ...
    <Card withBorder radius="lg" p={0} shadow="sm">
      <Group justify="space-between" p="xl" pb="md">
        <Title order={3} c={textColor}>
          {t("newsAdmin.table.title", "Danh sách tin tức")}
        </Title>
      </Group>
      ...
      <Group px="xl" pb="lg">
        <TextInput
          size="md"
          placeholder={t(
            "newsAdmin.table.searchPlaceholder",
            "Tìm kiếm tin tức..."
          )}
          ...
          <Table.Thead>
            <Table.Tr bg={theme === "dark" ? "dark.6" : "gray.1"}>
              ...
              <Table.Th style={{ width: 80 }} fw={700}>
                {t("newsAdmin.table.columns.image", "Ảnh")}
              </Table.Th>
              <Table.Th fw={700} style={{ width: "30%" }}>
                {t("newsAdmin.table.columns.title", "Tiêu đề")}
              </Table.Th>
              <Table.Th fw={700} style={{ width: "30%" }}>
                {t("newsAdmin.table.columns.subtitle", "Tiêu đề phụ")}
              </Table.Th>
              <Table.Th fw={700}>
                {t("newsAdmin.table.columns.featured", "Nổi bật")}
              </Table.Th>
              <Table.Th fw={700}>
                {t("newsAdmin.table.columns.status", "Trạng thái")}
              </Table.Th>
              <Table.Th ta="right" fw={700}>
                {t("newsAdmin.table.columns.actions", "Hành động")}
              </Table.Th>
            </Table.Tr>
          </Table.Thead>
          <Table.Tbody>
                  ...
                  <Table.Td>
                    {item.isFeatured ? (
                      <Badge color="yellow" variant="filled">
                        {t("newsAdmin.table.featured", "Nổi bật")}
                      </Badge>
                    ) : (
              ...
              <Table.Tr>
                <Table.Td colSpan={7}>
                  <Text c="dimmed" ta="center" py="xl" size="lg">
                    {t("newsAdmin.table.noData", "Không có tin tức nào.")}
                  </Text>
                </Table.Td>
              </Table.Tr>
            )}
      ...
      >
        <Text size="md" fw={500} c={textColor}>
          {t("common.total", "Tổng")}: {total} {t("newsAdmin.items", "tin tức")}{" "}
      ...
      <Modal
        ...
        title={
          <div className="text-center font-bold text-2xl">
            {t("newsAdmin.delete.title", "Xác nhận xoá")}
          </div>
        }
        centered
        size="sm"
      >
        <div className="px-2 py-10">
          <Text className="text-center">
            {t(
              "newsAdmin.delete.confirm",
              "Bạn có chắc chắn muốn xoá tin tức này? Hành động này không thể hoàn tác."
            )}
          </Text>
